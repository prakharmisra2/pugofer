...
ThreadSafeStream* o_pshared_char_stream;
...
void initializer(){
    ...
    o_pshared_char_stream = ThreadSafeStream_create();
...
void writeOutputStream(void* data, const char* type) {
    ...
    if (strcmp(type, "char") == 0) {
        ...
        printf("Writing to output stream: %c\n", data);
    } else if (strcmp(type, "int") == 0) {
        snprintf(buffer, sizeof(buffer), "%d", *((int*)data));
        printf("Writing to output stream: %d\n", data);
    } else if (strcmp(type, "str") == 0) {
        snprintf(buffer, sizeof(buffer), "%s", (char*)data);
        printf("Writing to output stream: %s\n", data);
    } else {
...
void readOutputStream(){
    ThreadSafeStream* stream = (ThreadSafeStream*)o_pshared_char_stream;
    int c = ThreadSafeStream_read(stream);
    EM_ASM({
        console.log("Reading from output stream: %s", UTF8ToString($0));
            //console.log(UTF8ToString($0));
        },c);
}